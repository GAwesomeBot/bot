					...
					if(checkFiltered(serverDocument, msg.channel, msg.cleanContent, false, true)) {
						...
						db.users.findOrCreate({_id: msg.author.id}, (err, userDocument) => {
							if(!err && userDocument) {
								...
								bot.handleViolation(winston, msg.guild, serverDocument, msg.channel, msg.member, userDocument, memberDocument, `You used a filtered word in #${msg.channel.name} on ${msg.guild.name}`, `**@${bot.getName(msg.guild, serverDocument, msg.member, true)}** used a filtered word (edited: \`${msg.cleanContent}\`) in #${msg.channel.name} on ${msg.guild.name}`, `Word filter violation (edited: "${msg.cleanContent}") in #${msg.channel.name}`, serverDocument.config.moderation.filters.custom_filter.action, serverDocument.config.moderation.filters.custom_filter.violator_role_id);
							} else {
