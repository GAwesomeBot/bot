...
const { RankScoreCalculator: computeRankScores, ModLog } = require("./Modules/");
...
module.exports = (db, configJS, configJSON) => {
	...
	if (configJS.shardTotal < 1) throw new RangeError(`In config.js, shardTotal must be greater than or equal to 1`);
	const bot = new Discord.Client({
		shardId: 0,
		shardCount: configJS.shardTotal - 1,
							...
							if (serverDocument.config.ranks_list[i].role_id) {
								...
								if (role) {
									try {
										...
										winston.error(`Failed to add member "${member.user.tag}" to role "${role.name}" on server "${server}" for rank level up`, { svrid: server.id, usrid: member.id, roleid: role.id }, err);
									}
								}
							}
						}
