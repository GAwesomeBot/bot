 ...
 * wanston, its used for initializing the global winston variable. Use that instead
...
winston.info("Logger has been loaded.");
...
(async () => {
	try {
		...
		winston.error(`There has been a database error! Is the database launched?\n`, err);
		process.exit(-1);
	}
})();
db = database.getConnection();
if (db) {
	winston.info(`Connected to the database successfuly.`);
