		...
		if(location) {
			try {
				weather.find({search: location, degreeType: unit}, (err, data) => {
					if(err) {
						winston.warn(`No weather data found for '${location}'`, {svrid: msg.guild.id, chid: msg.channel.id, usrid: msg.author.id});
						msg.channel.createMessage(`${msg.author.mention} I can't find weather info for ${location} 🌇`);
					} else {
						data = data[0];
						msg.channel.createMessage(`**${member ? (`Weather for @${bot.getName(msg.guild, serverDocument, member)}`) : data.location.name} right now:**\n${data.current.temperature}°${unit} ${data.current.skytext}, feels like ${data.current.feelslike}°, ${data.current.winddisplay} wind\n**Forecast for tomorrow:**\nHigh: ${data.forecast[1].high}°, low: ${data.forecast[1].low}° ${data.forecast[1].skytextday} with ${data.forecast[1].precip}% chance precip.`);
					}
				});
			} catch(err) {
				winston.error("Failed to get weather data", {svrid: msg.guild.id, chid: msg.channel.id, usrid: msg.author.id}, err);
