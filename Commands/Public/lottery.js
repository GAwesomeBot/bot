		...
		switch (suffix.toLowerCase().trim()) {
			case "start": {
				if (channelDocument.lottery.isOngoing) {
					const participantCount = channelDocument.lottery.participant_ids.filter((elem, i, self) => i === self.indexOf(elem)).length;
			...
			case "enroll": {
				if (channelDocument.lottery.isOngoing) {
					...
					if (userDocument.points >= ticketPrice) {
						const userTicketCount = channelDocument.lottery.participant_ids.reduce((n, usrid) => n + (usrid === msg.author.id), 0);
