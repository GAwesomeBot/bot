...
class GuildBanAdd extends BaseEvent {
	async handle (guild, user) {
		...
		if (!serverDocument) {
			return winston.warn("Failed to find server data for GuildBanAdd", { svrid: guild.id, usrid: user.id });
		}
		...
		if (serverDocument.config.moderation.isEnabled && serverDocument.config.moderation.status_messages.member_banned_message.isEnabled) {
			winston.verbose(`Member '${user.tag}' banned from guild '${guild}'`, { svrid: guild.id, usrid: user.id });
